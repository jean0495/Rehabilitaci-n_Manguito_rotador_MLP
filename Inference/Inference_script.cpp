/* Includes ---------------------------------------------------------------- */
#include <Proyecto_demo_inferencing.h>
...
{
    Serial.begin(9600);
    Serial1.begin(9600);        // TX/RX hacia microcontrolador de servos
    while (!Serial);
...
void loop()
{
    ...
    // Imprimir predicciones y encontrar la clase con mayor probabilidad
    float max_val = 0;
    uint8_t max_ix = 0;
    ...
    // Enviar classId por Serial1 solo si supera confianza mínima
    if (max_val >= CONFIANZA_MINIMA) {
        Serial1.println(max_ix);
        //ei_printf(">>> Enviado classId: %d (%s) confianza: %.3f\r\n",
            //max_ix, result.classification[max_ix].label, max_val);
    }
